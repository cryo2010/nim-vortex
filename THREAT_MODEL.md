...
| HTTP/2 framing floods (PING/SETTINGS/PRIORITY) | Per-connection control-frame budget (`maxControlFrames`) that decays on real progress in either direction (accepted requests and response DATA sent), GOAWAY when exceeded; WINDOW_UPDATE floods self-limit via the flow-control overflow check, and a connection-level or closed-stream update is only charged when no stream is open at all, so a legitimate client's flow control on a long download is never mistaken for a flood (#335) | `test_security_dos`, `test_http2_download` |
