...
| Threat | Defense | Verified by |
|--------|---------|-------------|
| HTTP/2 and HTTP/3 Rapid Reset (CVE-2023-44487) | Per-connection reset budget (`maxResetStreams`), GOAWAY(ENHANCE_YOUR_CALM) on HTTP/2 and `H3_EXCESSIVE_LOAD` on HTTP/3 when exceeded; a stream reset in the same read batch is never dispatched, so no handler or `blocking:` worker runs for a cancelled request | `test_security_dos` |
| HTTP/2 framing floods (PING/SETTINGS/PRIORITY) | Per-connection control-frame budget (`maxControlFrames`) that decays on real progress in either direction (accepted requests and response DATA sent), GOAWAY when exceeded; a WINDOW_UPDATE that unblocks nothing (connection-level while nothing waits on the connection window, or for a closed stream) spends credit earned by response DATA sent (one per 256 bytes, capped at 4x the budget) and is charged only past that, so a client's flow control on a long download is never mistaken for a flood while a peer we send nothing to still trips the budget (#335) | `test_security_dos`, `test_http2_download` |
...
| Slowloris / slow body / slow read | Coarse timeout wheel: `headerTimeout` (first byte → headers), `bodyTimeout` (*idle* during the body, re-armed on each read, so a stalled upload fires while an actively-transferring one on a slow link does not; it also arms while an HTTP/2 stream still awaits the client's head/body or the peer sits on an exhausted send window), `keepAliveTimeout` (idle between requests); optional `responseTimeout` and `writeTimeout` (idle while output is pending, so a client that never drains its response is cut off) | `test_security_dos` |
...
| Worker-pool saturation (slow or stuck `blocking:` endpoint) | `maxBlockingQueue` caps the tasks waiting for a free worker; past it the sync `blocking:` template answers 503 and the awaitable form raises `PoolSaturatedError`, so one slow endpoint cannot queue work without bound. `shutdownHardTimeout` bounds `close`/`waitFor` when a handler never returns (the thread is detached, not joined) | `test_blocking_pool` |
