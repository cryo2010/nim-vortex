...
import std/[unittest, net, strutils, os, tables, times, options]
...
withServer(handler, initVortexConfig(numThreads = 1), srv):
...
let (certFile, keyFile) = makeCertPair("nhs_cookie_certs_")

withServer(handler, initVortexConfig(
    numThreads = 1, workerThreads = 2, certFile = certFile,
    keyFile = keyFile), srvH3):
