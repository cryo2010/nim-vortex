...
import std/[unittest, net, httpcore, os, atomics, tables]
...
proc wsClient(op: int, payload: string, fin = true): string =
  ## A masked client WebSocket frame (RFC 6455) to place in a DATA payload.
  buildFrame(op, payload, fin)
...
proc parseWs(buf: string): (seq[WsMsg], int) =
  ...
  let (frames, used) = parseFrames(buf)
  for f in frames: result[0].add (f.op, f.fin, f.payload)
  result[1] = used
