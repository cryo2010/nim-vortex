import std/[net, macros]

macro fieldPassthrough(procDef: untyped): untyped =
  ## Fill a constructor proc's body with `ReturnType(p: p, ...)` derived from its
  ## own formal parameters, so the field list is stated once (the signature) plus
  ## once on the object type -- not a third time as a hand-maintained `field:
  ## field` block that every new setting must be added to in sync. The proc's
  ## written body (a placeholder `discard`) is replaced.
  procDef.expectKind nnkProcDef
  let formals = procDef.params
  let objConstr = nnkObjConstr.newTree(formals[0])   # formals[0] is the return type
  for i in 1 ..< formals.len:
    let idents = formals[i]                           # IdentDefs(name.., type, default)
    for j in 0 ..< idents.len - 2:                    # every name but the type + default
      objConstr.add nnkExprColonExpr.newTree(idents[j], idents[j])
  procDef.body = newStmtList(objConstr)
  result = procDef
...
proc initVortexConfig*(
    ...
    tlsCipherList = "",
    tlsCipherSuites = ""
): VortexConfig {.fieldPassthrough.} =
  ## Body generated by {.fieldPassthrough.}: constructs a VortexConfig mapping
  ## each parameter above to its like-named field. Keep every parameter name
  ## identical to its VortexConfig field.
  discard
