...
// not auto-decompress, so it sets Accept-Encoding: gzip and gunzips itself,
// proving the compression path.
//
// Run as a single-file source program: `java Interop.java`.
...
public class Interop {
    ...
    static byte[] maybeGunzip(HttpResponse<byte[]> resp) throws Exception {
        var enc = resp.headers().firstValue("content-encoding").orElse("");
        var body = resp.body();
        if (enc.equalsIgnoreCase("gzip") && body.length > 0) {
    ...
    static void one(HttpClient client, String base, String method, boolean mtls)
        ...
        var body = new String(maybeGunzip(resp));
        ...
        if (mtls) {
            ...
            var subject = new String(maybeGunzip(wres)).trim();
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.printf("INTEROP OK %s: requests=%d (clients=%d, runtime=%ds, mtls=%d)%n",
            NAME, count.get(), clients, runtime, mtls ? 1 : 0);
    }
}
